    ...
    <section id="connect" ref={containerRef} className="relative z-20 w-full min-h-screen bg-[#F7F7F7] text-[#111] flex flex-col justify-between pt-40 md:pt-48 pb-6 px-6 md:px-12 overflow-hidden">
      ...
      <div ref={topRef} className="flex flex-col md:flex-row justify-between items-start w-full gap-16 md:gap-8 z-10 flex-grow">
        ...
        <div className="flex flex-col items-start md:items-end w-full md:w-2/3">
          ...
          {/* 社交平台外链 (带下划线与箭头) */}
          <div className="flex flex-wrap gap-8 md:gap-12 mt-12 font-mono text-[11px] tracking-[0.2em] uppercase font-bold">
            {/* 你可以在这里继续添加其他社交平台 */}
          </div>
        </div>
      </div>
      ...
      {/* ================= 中部装饰性括号 ================= */}
      <div className="hidden md:flex justify-between items-center w-full px-8 mt-16 mb-4 font-mono text-[10px] tracking-[0.3em] opacity-30 z-10">
      ...
      {/* ================= 画册级页脚 ================= */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end w-full font-mono text-[8px] md:text-[9px] tracking-[0.2em] uppercase opacity-40 mt-8 pt-4 z-10">
